...
def vector_gaussian_noise(size: int, mean_noise: float = 0, std_dev_noise: float = 1.0) -> float:
...
class GaussianNoise:
    ...
    def add_noise(self, values: np.ndarray):
...
class AutoregressiveModelNoise:
    ...
    def __init__(self, model_param: float, std_dev_noise: float):
        ...
        self._std_dev_wn = math.sqrt(self._std_dev_noise ** 2 * (1 - self._model_param ** 2))
    ...
    def add_noise(self, values: np.ndarray):
        ...
        white_noise = np.random.normal(0, self._std_dev_wn, size=values2.shape)
